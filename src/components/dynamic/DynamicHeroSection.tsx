...
import { useHeroContent } from "@/hooks/useDynamicContent"
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          <div className="text-center space-y-8 animate-pulse">
            <div className="h-8 bg-muted rounded-full w-64 mx-auto"></div>
            <div className="space-y-4">
              <div className="h-16 bg-muted rounded w-full max-w-4xl mx-auto"></div>
              <div className="h-8 bg-muted rounded w-full max-w-3xl mx-auto"></div>
            </div>
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center space-y-8">
          ...
          <div className="space-y-4">
            <h1 className="text-4xl sm:text-6xl lg:text-7xl font-bold tracking-tight">
              {heroContent.title.split(' ').map((word, index) => {
                if (word.toLowerCase().includes('premium') || word.toLowerCase().includes('designs')) {
                  return (
                    <span key={index} className="gradient-text-youtube block">
                      {word}
                    </span>
                  );
                }
            ...
            {heroContent.subtitle && (
              <p className="text-xl sm:text-2xl text-muted-foreground max-w-3xl mx-auto leading-relaxed">
                ...
                {heroContent.description && (
                  <span className="text-foreground font-medium"> {heroContent.description}</span>
                )}
              </p>
            )}
          </div>
          ...
          {/* CTA buttons */}
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-8">
            <Link to={heroContent.cta_primary_link}>
              <Button 
                size="lg" 
                className="bg-gradient-youtube hover:shadow-glow transition-all duration-300 transform hover:scale-105 font-semibold text-lg px-8 py-4"
              >
            ...
            <Link to={heroContent.cta_secondary_link}>
              <Button 
                variant="outline" 
                size="lg"
                className="border-2 border-youtube-red text-youtube-red hover:bg-youtube-red hover:text-white font-semibold text-lg px-8 py-4 transition-smooth"
